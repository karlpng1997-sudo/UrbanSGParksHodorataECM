...
taxa <- assignTaxonomy(rep_seqs, "C:/Users/karlp/OneDrive/Desktop/Rcode/sh_general_release_dynamic_10.10.2017.fasta", multithread = TRUE)
...
otu_cca <- read.csv("OTU_table_ECM_and_nonECM_CCA.csv")
...
merged <- inner_join(otu_cca, env, by = "sample")

# Extract OTU matrix and env variables

otu_matrix <- merged %>% select(contains("OTU"))
...
cca_full <- cca(otu_matrix ~ pH + clay + sand + Condition(site), data = env_data)
...
cols <- palette.colors(nlevels(env$site), "Tableau 10")
ordiplot(cca_full, type = "n", main = "Partial CCA", xlab = "CCA1 (65.9%)",ylab = "CCA2 (26.2%)")
ordiellipse(cca_full,
           groups = env$site,
           kind = "sd",
           conf = 0.95,
           col = cols,
...
#      cex = 0.7,
#      col = "black")
envfit_res <- envfit(cca_full,
                     env_data[, c("pH", "clay", "sand")],
                     permutations = 999)
plot(envfit_res, col = "red")
legend("topright",
       legend = levels(env$site),
       col = cols,
       pch = 19,
       lwd = 2,
       bty = "n")
...
cca_null <- cca(otu_matrix ~ Condition(site), data = env_data)
...
env$site <- factor(env$site)
# ordiellipse(cca_step,
#             groups = env$site,
#             kind = "sd",
#             conf = 0.95,
#             col = 1:length(levels(env$site)))
points(cca_step,
       display = "sites",
       pch = 19,
       col = cols[env$site])
# text(cca_step, display = "species", cex = 0.7, col = "black")
...
# Load relevant packages for ANOSIM
...
otu <- read.csv("OTU_table_ECM_and_nonECM_CCA.csv") 

env <- read.csv("ECMASV_GLMM_OTU.csv")

# Merge datasets by sample ID

merged <- inner_join(otu, env, by = "sample")
...
row_sums <- rowSums(otu_data)
merged <- merged[row_sums > 0, ]
otu_data <- otu_data[row_sums > 0, ]
